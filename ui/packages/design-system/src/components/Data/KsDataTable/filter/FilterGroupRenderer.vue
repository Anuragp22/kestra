    ...
    >
        <template v-for="(childLeaf, childIndex) in unit.children" :key="childLeaf.id">
            ...
            <div
                ...
                @dragover="(e) => $emit('drag-over', e, childLeaf.id)"
                @dragleave="(e) => $emit('drag-leave', e, childLeaf.id)"
                @drop="(e) => $emit('drop', e, childLeaf.id)"
            >
                <span
...
.filter-wrapper-container {
    ...
    &.drag-over {
        box-shadow: 0 0 0 2px var(--ks-border-primary);
    }
