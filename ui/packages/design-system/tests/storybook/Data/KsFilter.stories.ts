...
/**
 * A top-level OR group whose first slot holds an AND-wrapper combining two
 * conditions: `(state IN [RUNNING] AND flowId = full) OR namespace ^= io.kestra`.
 * Demonstrates one level of nesting — the chip UI's maximum before falling back
 * to the raw editor.
 */
export const WithANDWrapper: Story = {
    render: () => ({
        components: {KsFilter},
        setup() {
            const ready = useIsolatedRouter({
