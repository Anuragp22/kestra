            ...
            .trigger(
                ExecutionTrigger.of(
                    ...
                    Output.builder()
                        ...
                        .firstFailedTaskId(ListUtils.emptyOnNull(current.getTaskRunList()).stream().filter(t -> t.getState().getCurrent().isFailed()).map(TaskRun::getId).findFirst().orElse(null))
                        .lastTaskId(ListUtils.emptyOnNull(current.getTaskRunList()).reversed().stream().map(TaskRun::getId).findFirst().orElse(null))
                        .outputs(outputs)
                        .build()
                )
            );
